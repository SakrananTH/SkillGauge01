...
const AdminQuizBank = () => {
  ...
  const [assessmentDurationMinutes, setAssessmentDurationMinutes] = useState(() => {
  ...
  const [assessmentDurationMessage, setAssessmentDurationMessage] = useState('');
    ...
    try {
      ...
      try {
        ...
        const converted = structItems.map(item => ({
          ...
          category: 'structure',
          subcategory: item.skill_type || null,
  ...
  useEffect(() => {
    ...
    try {
      window.localStorage.setItem(ASSESSMENT_DURATION_STORAGE_KEY, String(assessmentDurationMinutes));
      setAssessmentDurationMessage('บันทึกเวลาสอบแล้ว');
      const t = window.setTimeout(() => setAssessmentDurationMessage(''), 2000);
            ...
            <div className="form-grid form-grid--round">
              <div className="form-group form-group--compact">
                ...
                <div className="form-group">
                  <label htmlFor="assessment-duration-minutes">เวลาทำข้อสอบ (นาที)</label>
                  <input
                    id="assessment-duration-minutes"
                    type="number"
                    min={1}
                    max={240}
                    ...
                    onChange={(event) => {
                      ...
                      setAssessmentDurationMinutes(nextValue);
                    }}
                  />
                  <div className="form-hint">ค่าเริ่มต้น {DEFAULT_ASSESSMENT_DURATION_MINUTES} นาที (ใช้กับหน้า Worker ตอนทำข้อสอบ)</div>
                  {assessmentDurationMessage && <div className="form-feedback success">{assessmentDurationMessage}</div>}
                </div>
              </div>
          ...
          <div className="quiz-form-card quiz-table-card">
            ...
            <div className="quiz-table-controls">
              ...
              <div className="quiz-table-filters">
                {SUBCATEGORY_OPTIONS[roundForm.category] && SUBCATEGORY_OPTIONS[roundForm.category].length > 0 && (
                  <div className="filter-control">
                    <label htmlFor="table-subcategory-filter">หมวดหมู่</label>
                    <select
                    ...
                    >
                      ...
                      {SUBCATEGORY_OPTIONS[roundForm.category].map(option => (
                ...
                <table className="quiz-table">
                  ...
                  <tbody>
                    {displayedQuestions.map((question, index) => {
                      ...
                      const subcategoryLabel = question.subcategory && SUBCATEGORY_OPTIONS[roundForm.category]
                        ? SUBCATEGORY_OPTIONS[roundForm.category].find(opt => opt.value === question.subcategory)?.label || 'ไม่พบหมวดหมู่'
                        : 'ไม่ระบุหมวดหมู่';
                      return (
                        <tr key={question.id}>
                          <td>{index + 1}</td>
                    ...
                    {displayedQuestions.length === 0 && (
                      <tr>
                        <td colSpan={SUBCATEGORY_OPTIONS[roundForm.category] && SUBCATEGORY_OPTIONS[roundForm.category].length > 0 ? "7" : "6"} className="table-empty">
